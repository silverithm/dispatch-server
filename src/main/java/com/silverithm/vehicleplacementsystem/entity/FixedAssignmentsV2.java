...
@Getter
public class FixedAssignmentsV2 {

    private final Map<Integer, List<Integer>> fixedAssignments;

    public FixedAssignmentsV2(List<FixedAssignmentsDTO> fixedAssignmentDtos, List<EmployeeDTO> employees,
    ...
    public boolean evaluateFitness(ChromosomeV3 chromosome) {
        for (int employee_idx : fixedAssignments.keySet()) {
            for (int i = 0; i < chromosome.getGenes()[employee_idx].length; i++) {
                // 고정 배정과 다르고, 고정 배정이 -1이 아닌 경우
                if (chromosome.getGenes()[employee_idx][i] != fixedAssignments.get(employee_idx).get(i)
