...
@Service
@EnableCaching
public class GeneticAlgorithmV4 {


    private static final Random rand = new Random();

    private final StringBuilder elderlyBuilder = new StringBuilder("Elderly_");
    private final StringBuilder employeeBuilder = new StringBuilder("Employee_");
    private static final String COMPANY = "Company";  // 자주 사용되는 상수
    ...
    public GeneticAlgorithmV4(List<EmployeeDTO> employees,
...
//        log.info("total departure time : " + totalDepartureTime + ", fitness : " + fitness);
        ...
        if (dispatchType == DispatchType.DURATION_IN || dispatchType == DispatchType.DURATION_OUT) {
            for (int i = 0; i < chromosome.getGenes().length; i++) {
                for (int j = 0; j < chromosome.getGenes()[i].length - 1; j++) {
                    ...
                    elderlyBuilder.setLength(8); // "Elderly_" 길이로 리셋
                    elderlyBuilder.append(elderlys.get(elderlyIndex1).id());
                    String elderly1 = elderlyBuilder.toString();

                    // 두 번째 elderly ID 생성
                    elderlyBuilder.setLength(8);
                    elderlyBuilder.append(elderlys.get(elderlyIndex2).id());
                    String elderly2 = elderlyBuilder.toString();

                    fitness += calculateFitnessForFromAndTo(elderly1, elderly2);
                }
        ...
        if (dispatchType == DispatchType.DISTANCE_IN || dispatchType == DispatchType.DISTANCE_OUT) {
            for (int i = 0; i < chromosome.getGenes().length; i++) {
                for (int j = 0; j < chromosome.getGenes()[i].length - 1; j++) {
                    ...
                    if (calculateFitnessForFromAndTo(elderly1, elderly2) == 10000) {
                        fitness += 10000;
                    } else {
                        // Employee ID 생성
                        employeeBuilder.setLength(9); // "Employee_" 길이로 리셋
                        employeeBuilder.append(employees.get(i).id());

                        fitness += calculateFitnessForFromAndTo(employeeBuilder.toString(), elderly1);
                    }
    ...
    private double addFitnessForDispatchTypes(ChromosomeV3 chromosome, double fitness, int i) {
        ...
        if (dispatchType.equals(DispatchType.DISTANCE_OUT) || dispatchType.equals(DispatchType.DURATION_OUT)) {
            ...
            elderlyBuilder.setLength(8);
            elderlyBuilder.append(elderlys.get(chromosome.getGenes()[i][chromosome.getGenes()[i].length - 1]).id());
            String lastElderly = elderlyBuilder.toString();

            if (employees.get(i).isDriver()) {
                fitness += calculateFitnessForFromAndTo(lastElderly, COMPANY);
            } else {
                employeeBuilder.setLength(9); // "Employee_" 길이로 리셋
                employeeBuilder.append(employees.get(i).id());
                fitness += calculateFitnessForFromAndTo(lastElderly, employeeBuilder.toString());
            }
        ...
        if (dispatchType.equals(DispatchType.DURATION_IN) || dispatchType.equals(DispatchType.DISTANCE_IN)) {
            ...
            elderlyBuilder.setLength(8);
            elderlyBuilder.append(elderlys.get(chromosome.getGenes()[i][0]).id());
            String firstElderly = elderlyBuilder.toString();

            if (employees.get(i).isDriver()) {
                fitness += calculateFitnessForFromAndTo(COMPANY,
                        firstElderly);
            } else {
                employeeBuilder.setLength(9);
                employeeBuilder.append(employees.get(i).id());
                fitness += calculateFitnessForFromAndTo(employeeBuilder.toString(), firstElderly);
            }

            // 마지막 노인 ID 생성
            elderlyBuilder.setLength(8);
            elderlyBuilder.append(elderlys.get(chromosome.getGenes()[i][chromosome.getGenes()[i].length - 1]).id());
            fitness += calculateFitnessForFromAndTo(elderlyBuilder.toString(), COMPANY);
        ...
        if (dispatchType.equals(DispatchType.DISTANCE_OUT) || dispatchType.equals(DispatchType.DURATION_OUT)) {
            for (int i = 0; i < chromosome.getGenes().length; i++) {
                ...
                for (int j = 0; j < chromosome.getGenes()[i].length - 1; j++) {
                    ...
                    // 현재 노인 ID 생성
                    elderlyBuilder.setLength(8);
                    elderlyBuilder.append(elderlys.get(chromosome.getGenes()[i][j]).id());
                    String startNodeId = elderlyBuilder.toString();

                    // 다음 노인 ID 생성
                    elderlyBuilder.setLength(8);
                    elderlyBuilder.append(elderlys.get(chromosome.getGenes()[i][j + 1]).id());
                    String destinationNodeId = elderlyBuilder.toString();
